...
/**
 * Sucht Karten anhand eines (Teil-)Namens.
 * - Mit setId: durchsucht nur dieses Set (schnell).
 * - Ohne setId (freie Sammlung): durchsucht alle bekannten Sets nacheinander
 *   (nutzt Cache, kann beim allerersten Mal etwas dauern). Bricht bei 30 Treffern ab.
 */
export async function searchCards(nameQuery, { setId } = {}) {
  const clean = String(nameQuery).trim().toLowerCase();
  if (!clean) return [];
  ...
  if (setId) {
    ...
    for (const raw of cards) {
      ...
      if (name.toLowerCase().includes(clean)) results.push(normalizeCard(raw, setId));
    }
  ...
  for (const s of sets) {
    ...
    try {
      const cards = await fetchSetCards(s.id);
      for (const raw of cards) {
        if (results.length >= 30) break;
        const name = pick(raw, ["card_name", "name"]) || "";
        if (name.toLowerCase().includes(clean)) results.push(normalizeCard(raw, s.id));
      }
    } catch {
      /* einzelnes Set übersprungen, falls nicht ladbar */
    }
  }
  return results;
}
